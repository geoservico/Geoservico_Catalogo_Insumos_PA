...
class LayerLoadWorker(QThread):
    ...
    def run(self):
        ...
        for i, layer_name in enumerate(self.layers):
            try:
                # Configurar autenticação via cabeçalhos HTTP ao invés de URL
                uri = f"url=http://srv1185637.hstgr.cloud:8082/geoserver/ASSINATURA_PA/wms?&crs=EPSG:4674&format=image/png&layers={layer_name}&styles=&username={self.usuario}&password={self.senha}"
                raster_layer = QgsRasterLayer(uri, layer_name.replace("ASSINATURA_PA:", ""), "wms")
...
class GeoservicoInsumosPlugin:
    ...
    def show_layer_list(self):
        ...
        def on_selection_changed():
            btn_carregar.setEnabled(len(list_widget.selectedItems()) > 0)
        ...
        def carregar():
            ...
            if not self.is_user_authenticated() or not self.validate_authentication():
                QMessageBox.warning(None, "Sessão Expirada", 
            ...
            for item in list_widget.selectedItems():
                # Extrair o nome real da camada do texto do item
                item_text = item.text()
                layer_name = item_text.split('(')[-1].rstrip(')')
                selecionadas.append(layer_name)
            ...
            self.worker = LayerLoadWorker(selecionadas, self.usuario, self.senha)
            self.worker.progress.connect(progress_bar.setValue)
            self.worker.finished.connect(lambda loaded: self.on_layers_loaded(loaded, dlg, progress_frame, btn_carregar))
            self.worker.error.connect(lambda error: QgsMessageLog.logMessage(error, "Geoserviço", Qgis.Warning))
            self.worker.start()
        ...
        btn_carregar.clicked.connect(carregar)
    ...
    def logout_user(self, dialog=None):
        ...
        # Limpar credenciais salvas se solicitado
        reply = QMessageBox.question(None, "Logout", 
                                   "Deseja também remover o usuário salvo? (A senha não é salva por segurança)",
